...
def extract_content():
    ...
    try:
        ...
        logger.info(f"Conversion result repr: {repr(result)}")
        
        # Try to extract properties from the result object
        try:
            # If result has attributes, log them
            for attr in dir(result):
                if not attr.startswith('_'):  # Skip private/special attributes
                    try:
                        value = getattr(result, attr)
                        if callable(value):  # Skip methods
                            continue
                        logger.info(f"Result attribute {attr}: {value}")
                    except Exception as e:
                        logger.warning(f"Error accessing attribute {attr}: {e}")
        except Exception as e:
            logger.warning(f"Error inspecting result object: {e}")
        
        # Extract text content from the result object
        # The result is a DocumentConverterResult object
...
if __name__ == "__main__":
    # Run the Flask app directly when script is executed
